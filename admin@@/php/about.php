...
function about($db)
{
	...
	if(isset($_POST["addNew"])) {
		...
		try{
            ...
            if(common::file_check($_FILES['file'])){
                WideImage::load('file')->resize(256,204, 'fill')->saveToFile(myPath.$file);
                $db->where('id',$recent);
    ...
    $str.=$form->search_area($db,$act,'about_cate',$_GET['hint'],1);
    ...
    if($db->count!=0){
        foreach($list as $item){
            $cate=$db->where('id',$item['pId'])->getOne('about_cate','id,title');
            $item_content = array(
                array($item['title'],'text'),
