...
function product($db){
	...
	if(isset($_POST["addNew"])) {
        $insert = array(
                    ...
                    'price'=>$price,'lnk'=>$lnk,'pId'=>$pId,
                    'meta_keyword'=>$meta_kw,'meta_description'=>$meta_desc,
                    'home'=>$home,'active'=>$active
                );
		try{
            $recent=$db->insert($table,$insert);
            if(common::file_check($_FILES['file'])){
                WideImage::load('file')->resize(210,216, 'fill')->saveToFile(myPath.$file);
                $db->where('id',$recent);
    ...
    $head_title=array('Tên SP','Giá(VNĐ)','Hình ảnh','Danh mục','Trang chủ','Hiển thị');
	$str.=$form->table_start($head_title);
    ...
    if($db->count!=0){
        $db_sub=$db;
        foreach($list as $item){
            $cate=$db->where('id',$item['pId'])->where('lev',1)->getOne('product_cate','id,title');
            //$img=$db->where('pId',$item['id'])->orderBy('ind','asc')->getOne('product_image','img');
            if(trim($item['img'])==='') $img='holder.js/130x100';else $img=myPath.$item['img'];   
            $item_content = array(
                array($item['title'],'text'),
	...
	<div class="row">
        ...
        <div class="col-lg-12">
            ...
            '.$form->number('price',array('label'=>'Đơn giá')).'
            '.$form->text('lnk',array('label'=>'Link demo<code>http://</code>','required'=>true)).'    
            '.$form->text('meta_keyword',array('label'=>'Keyword <code>SEO</code>')).'
            '.$form->textarea('meta_description',array('label'=>'Description <code>SEO</code>')).'
            '.$form->ckeditor('content',array('label'=>'Chi tiết')).'
            '.$form->file('file',array('label'=>'Hình ảnh<code>210,216</code>')).'
            '.$form->checkbox('active',array('label'=>'Hiển Thị','checked'=>true)).'
...
function product_image($db){
	...
	if(isset($_POST["addNew"])) {
		...
		try{
            $recent = $db->insert($table,$insert);
            if(common::file_check($_FILES['file'])){
                WideImage::load('file')->resize(800,800, 'fill')->saveToFile(myPath.$file);
                WideImage::load(myPath.$file)->resize(400,400, 'fill')->saveToFile(myPath.'thumb_'.$file);
                $db->where('id',$recent);
                $db->update($table,array('img'=>$file));
            }
            header("location:".$_SERVER['REQUEST_URI'],true);
	...
	<div class="row">
    	<div class="col-lg-12"><h3>Cập nhật - Thêm mới thông tin</h3></div>
        <div class="col-lg-12">
            '.$form->file('file',array('label'=>'Hình ảnh <code>( 800,800 )</code>')).'
            '.$form->number('ind',array('label'=>'Thứ tự','required'=>true)).'
            '.$form->checkbox('active',array('label'=>'Hiển Thị','checked'=>true)).'
        </div>
    	'.$form->hidden($btn['name'],$btn['value']).'
