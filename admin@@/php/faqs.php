...
function faqs($db)
{
	...
	if(isset($_POST["addNew"])) {
        $insert = array(
            ...
            'active'=>$active,'dates'=>date('Y-d-m H:i:s')
        );
		try{
            $recent = $db->insert($table,$insert);
            /*if(common::file_check($_FILES['file'])){
                WideImage::load('file')->resize(85,85, 'fill')->saveToFile(myPath.$file);
                $db->where('id',$recent);
	...
	<div class="row">
    	<div class="col-lg-12"><h3>Cập nhật - Thêm mới thông tin</h3></div>
        <div class="col-lg-12">
            '.$form->textarea('title',array('label'=>'Tiêu đề','required'=>true)).'
